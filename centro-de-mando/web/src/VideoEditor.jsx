...
export default function VideoEditor({ data, ws, flash, editorSrc, onPublicar }) {
  ...
  const [efectoClip, setEfectoClip] = useState("none");   // none|fade|zoom|bn|dorado|splash|anaglifo|glitch
  ...
  const [fondoTexto, setFondoTexto] = useState("none");   // none | resplandor | caja
  const [colorTexto, setColorTexto] = useState("cream");  // cream | oro
  const [fuenteTexto, setFuenteTexto] = useState("display"); // display (bold) | elegante (light) | mono
  ...
  const galeriaVids = (data[ws].gc || []).filter((x) => x.video || x.tipo === "video").map((x) => ({ url: x.video || x.imagen, nombre: (x.prompt || "video").slice(0, 40) })).filter((x) => x.url);
  ...
  function drawFrame(ctx, v, w, h, prog) {
    ...
    if (ef === "anaglifo") {
      ...
      // tinte champagne sutil en una banda
      ctx.globalCompositeOperation = "screen"; ctx.fillStyle = ORO; ctx.globalAlpha = 0.14;
      const gy = ((prog * 3) % 1) * h; ctx.fillRect(0, gy, w, h * 0.06); ctx.globalAlpha = 1; ctx.globalCompositeOperation = "source-over";
    } else {
      if (ef === "bn" || ef === "dorado" || ef === "splash") ctx.filter = "grayscale(1) contrast(1.05)";
      drawCoverTo(ctx, v, w, h, zoom);
      ctx.filter = "none";
      if (ef === "dorado") {
        ctx.globalCompositeOperation = "multiply"; ctx.fillStyle = ORO; ctx.fillRect(0, 0, w, h);
        ctx.globalCompositeOperation = "lighten"; ctx.fillStyle = "rgba(30,25,18,0.25)"; ctx.fillRect(0, 0, w, h);
        ctx.globalCompositeOperation = "source-over";
      }
      if (ef === "splash") {
        // círculo central a color (con leve champagne de marca) sobre el B&N
        ctx.save();
        const cx = w / 2, cy = h / 2, r = Math.min(w, h) * 0.34;
        ctx.beginPath(); ctx.arc(cx, cy, r, 0, Math.PI * 2); ctx.clip();
        drawCoverTo(ctx, v, w, h, zoom);
        ctx.globalCompositeOperation = "soft-light"; ctx.fillStyle = ORO; ctx.globalAlpha = 0.22; ctx.fillRect(0, 0, w, h);
  ...
  function dibujarTexto(ctx, w, h, prog) {
    ...
    const maxW = w * 0.84;
    // tipografía de marca: display (Inter bold), elegante (Inter light,
    // aireada) o mono (mayúsculas, estilo footer).
    const esMono = fuenteTexto === "mono", esEleg = fuenteTexto === "elegante";
    const fam = esMono ? "ui-monospace, monospace" : "Inter, system-ui, sans-serif";
    const weight = esMono ? 500 : (esEleg ? 300 : 700);
    const col = colorTexto === "oro" ? "#C6A87F" : "#F5F2EC";
    ...
    if (fondoTexto === "caja") {
      ...
      ctx.fillStyle = "rgba(11,10,8,0.42)";
    ...
    if (fondoTexto === "resplandor") {
      // resplandor champagne de marca (varias pasadas) + sombra oscura sutil + texto nítido encima
      ctx.shadowColor = "rgba(198,168,127,0.95)"; ctx.shadowBlur = Math.round(fs * 0.9); ctx.shadowOffsetY = 0;
      pinta(); pinta();
      ctx.shadowColor = "rgba(8,7,5,0.55)"; ctx.shadowBlur = Math.round(fs * 0.22); ctx.shadowOffsetY = Math.max(1, Math.round(fs * 0.03));
      pinta();
    } else {
      // sombra suave (o encima de la caja): legibilidad
      if (fondoTexto !== "caja") { ctx.shadowColor = "rgba(8,7,5,0.72)"; ctx.shadowBlur = Math.round(fs * 0.42); ctx.shadowOffsetY = Math.max(1, Math.round(fs * 0.04)); }
      pinta();
    }
  ...
  function descargar() { if (!salidaUrl) return; const a = document.createElement("a"); a.href = salidaUrl; a.download = "atlantis-video.webm"; a.click(); }
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  {[["none", "Ninguno"], ["fade", "Fundido"], ["zoom", "Zoom lento"], ["bn", "B&N"], ["dorado", "Champagne"], ["splash", "Splash champagne"], ["anaglifo", "Anaglifo 3D"], ["glitch", "Glitch"]].map(([id, l]) => (
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                  ...
                  {[["display", "Título"], ["elegante", "Elegante"], ["mono", "Código"]].map(([id, l]) => (
                    <button key={id} onClick={() => setFuenteTexto(id)} title={id === "mono" ? "Monoespaciada en mayúsculas" : id === "elegante" ? "Inter light, aireada y fina" : "Inter bold, titular"} style={{ background: fuenteTexto === id ? C.aetherSoft : "transparent", border: `1px solid ${fuenteTexto === id ? C.aetherLine : C.line}`, color: fuenteTexto === id ? C.aether2 : C.ash, fontFamily: id === "mono" ? "ui-monospace, monospace" : "Inter, sans-serif", fontWeight: id === "elegante" ? 300 : (id === "display" ? 700 : 500) }} className="px-2 py-1 rounded fs-10">{l}</button>
                  ))}
                </div>
                {/* color y estilo del texto */}
                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                  <span style={{ color: C.ash, fontFamily: MONO }} className="fs-9 uppercase mr-0.5">Color</span>
                  {[["cream", "Claro"], ["oro", "Champagne"]].map(([id, l]) => (
                    <button key={id} onClick={() => setColorTexto(id)} style={{ background: colorTexto === id ? C.aetherSoft : "transparent", border: `1px solid ${colorTexto === id ? C.aetherLine : C.line}`, color: id === "oro" ? "#C6A87F" : (colorTexto === id ? C.aether2 : C.ash) }} className="px-2 py-1 rounded fs-10">{l}</button>
                  ))}
                </div>
                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
