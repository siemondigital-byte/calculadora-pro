...
import { MOTOR, getToken, guardarSecreto, estadoSecretos } from "./db.js";
...
export default function EstudioContenidoView({ data, commit, ws, flash, onPublicar, estudioDraft, clearEstudioDraft, irAEditor }) {
  ...
  const galeria = data[ws].gc || [];
  const patchGc = (nuevos) => commit({ ...data, [ws]: { ...data[ws], gc: nuevos } });
  // modo diseño (diseño SVG editable en el sistema visual de la marca)
  ...
  // inspiración de titulares: 4 propuestas con ángulos distintos
  ...
  // integración: llega una tendencia/idea desde otro módulo → precarga campos
  useEffect(() => {
    ...
    flash("Idea cargada. Ajusta y genera la pieza.");
  ...
  function descargarPng() {
    ...
    try {
      ...
      img.onload = () => {
        ...
        const a = document.createElement("a"); a.download = "atlantis-diseno.png"; a.href = cv.toDataURL("image/png"); a.click();
      };
  ...
  function descargarSvg() {
    ...
    const a = document.createElement("a"); a.download = "atlantis-diseno.svg"; a.href = URL.createObjectURL(blob); a.click();
  }
  function guardarDiseno() {
    if (!svg) return;
    patchGc([{ id: uid(), tipo: "diseno", estado: "listo", svg, w: svgDim.w, h: svgDim.h, prompt: dTitulo, fecha: today() }, ...galeria]);
  ...
  function descargarSlide(s) {
    svgToPng(s.svg, s.w, s.h).then((u) => { const a = document.createElement("a"); a.download = `atlantis-carrusel-${String(s.n).padStart(2, "0")}.png`; a.href = u; a.click(); }).catch(() => flash("No pude exportar la lámina."));
  }
  async function descargarCarrusel(sl) {
    const arr = sl || slides;
    for (const s of arr) {
      // eslint-disable-next-line no-await-in-loop
      await new Promise((res) => { svgToPng(s.svg, s.w, s.h).then((u) => { const a = document.createElement("a"); a.download = `atlantis-carrusel-${String(s.n).padStart(2, "0")}.png`; a.href = u; a.click(); setTimeout(res, 400); }).catch(res); });
    }
  ...
  useEffect(() => { (async () => {
    try { const d = await estadoSecretos(); setFalOn(!!(d.FAL_API_KEY && d.FAL_API_KEY.valido)); } catch {}
  })(); }, []);

  async function guardarFal() {
    if (!falKey.trim()) return;
    const ok = await guardarSecreto("FAL_API_KEY", falKey.trim());
    if (ok) { setFalKey(""); setFalOn(true); setShowCfg(false); flash("FAL key guardada en el servidor."); }
    else flash("No pude guardar la key.");
  }
  ...
  async function generarImagen() {
    ...
    try {
      ...
      else flash(d.error === "sin_fal_key" ? "Falta tu FAL key (Conectar FAL)." : "No pude generar: " + (d.error || ""));
    } catch { flash("No pude conectar con el motor."); }
  ...
  async function buscarBanco() {
    ...
    try {
      ...
      const d = await r.json();
      if (d.sinKey || d.error === "sin_key") { flash("Conecta un banco de fotos (Pexels) en Accesos primero."); setBancoRes({ fotos: [] }); }
  ...
  async function buscarBancoVideo() {
    ...
    try {
      ...
      const d = await r.json();
      if (d.ok) { setVidRes({ videos: d.videos || [] }); if (!(d.videos || []).length) flash("Sin videos para esa búsqueda. Prueba otras palabras (o en inglés como 'city', 'skyline')."); }
  ...
  function borrar(id) { patchGc(galeria.filter((x) => x.id !== id)); }
  ...
  async function regenerar(x) {
    ...
    setRegId(x.id);
    const nuevo = (obj) => patchGc([{ id: uid(), fecha: today(), prompt: tema, estado: "listo", ...obj }, ...galeria]);
    try {
      if (x.tipo === "imagen") {
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      let cambio = false; const actual = data[ws].gc || [];
      ...
      if (cambio) patchGc(upd);
    }, 8000);
    return () => { if (pollRef.current) { clearInterval(pollRef.current); pollRef.current = null; } };
    // eslint-disable-next-line
  }, [galeria.map((x) => x.id + x.estado).join(",")]);

  const copiar = (u) => { try { navigator.clipboard.writeText(u); flash("URL copiada. Pégala en Publicar (o como ad)."); } catch {} };
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      ...
      <div className="flex gap-2 mb-4">
        ...
        <button onClick={() => setModo("diseno")} style={{ background: modo === "diseno" ? C.aetherSoft : C.panel, border: `1px solid ${modo === "diseno" ? C.aetherLine : C.line}`, color: modo === "diseno" ? C.aether2 : C.mist }} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg fs-12"><Palette size={14} /> Diseño editable</button>
      ...
      {modo === "carrusel" && (
        <div style={{ background: C.panel, border: `1px solid ${C.line}` }} className="rounded-xl p-4 md:p-5 mb-5">
          <div style={{ color: C.mist }} className="fs-12 mb-3">Escribe tu mensaje y el sistema arma un carrusel completo en la voz de la marca: gancho, desarrollo y cierre, todas las láminas con el mismo estilo. Se guarda solo en la galería.</div>
          <label style={{ color: C.ash, fontFamily: MONO }} className="fs-10 uppercase">Mensaje / idea del carrusel</label>
          <textarea style={{ ...inS, minHeight: 70, resize: "vertical" }} value={cMensaje} onChange={(e) => setCMensaje(e.target.value)} placeholder="ej. Juntar dinero sin un plan también tiene un costo. La diferencia la marca la estructura." />
        ...
        <div style={{ background: C.panel, border: `1px solid ${C.line}` }} className="rounded-xl p-4 md:p-5 mb-5">
          <div className="grid md:grid-cols-2 gap-3">
            <div className="md:col-span-2">
              ...
              <input style={inS} value={dTitulo} onChange={(e) => setDTitulo(e.target.value)} placeholder="ej. Arquitectos de patrimonio" /></div>
            {propuestas.length > 0 && (
            ...
            <div className="md:col-span-2"><label style={{ color: C.ash, fontFamily: MONO }} className="fs-10 uppercase">Subtítulo</label>
              <input style={inS} value={dSub} onChange={(e) => setDSub(e.target.value)} placeholder="ej. Patrimonio inmobiliario con estructura, no con suerte" /></div>
          ...
          <div style={{ borderTop: `1px solid ${C.line}` }} className="mt-4 pt-4">
            ...
            <div className="flex gap-2 mt-1.5">
              <input style={{ ...inS, fontSize: 13 }} value={bancoQ} onChange={(e) => setBancoQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && buscarBanco()} placeholder="ej. edificio moderno, skyline, arquitectura" />
            ...
            <div style={{ color: C.ash }} className="fs-10 mt-2">Clic en una foto para abrir el editor (filtros + 3 textos). ¿Prefieres una imagen con IA? Genérala en <b style={{ color: C.mist }}>“Foto y video (FAL)”</b> y edítala igual.</div>
          </div>
        </div>
      )}
      ...
      <div style={{ background: C.panel, border: `1px solid ${C.line}` }} className="rounded-xl p-4 md:p-5 mb-5">
        ...
        <textarea style={{ ...inS, minHeight: 70, resize: "vertical" }} value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="ej. una profesional revisando planos de un proyecto inmobiliario, ambiente elegante, tonos oscuros cálidos y champagne" />
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3">
          <div><label style={{ color: C.ash, fontFamily: MONO }} className="fs-10 uppercase">Formato</label>
        ...
        {/* banco de imágenes gratis (Pexels/Pixabay/Unsplash) */}
        <div style={{ borderTop: `1px solid ${C.line}` }} className="mt-4 pt-4">
          ...
          <div className="flex gap-2 mt-1.5">
            <input style={{ ...inS, fontSize: 13 }} value={bancoQ} onChange={(e) => setBancoQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && buscarBanco()} placeholder="ej. edificio en construcción, ciudad, arquitectura moderna" />
            <button onClick={buscarBanco} disabled={bancoBusy} style={{ background: C.carbon, border: `1px solid ${C.aetherLine}`, color: C.aether2 }} className="px-3 rounded-lg fs-12 shrink-0">{bancoBusy ? "…" : "Buscar"}</button>
          </div>
          {bancoRes && (bancoRes.fotos || []).length > 0 && (
            <div className="flex gap-2 overflow-x-auto pb-1 mt-2">
          ...
          <div className="flex gap-2 mt-1.5">
            <input style={{ ...inS, fontSize: 13 }} value={vidQ} onChange={(e) => setVidQ(e.target.value)} onKeyDown={(e) => e.key === "Enter" && buscarBancoVideo()} placeholder="ej. ciudad de noche, obra en construcción, skyline" />
          ...
          {vidRes && !(vidRes.videos || []).length && <div style={{ color: C.ash }} className="fs-10 mt-2">Sin videos para esa búsqueda. Prueba en inglés (ej. "city", "skyline", "construction").</div>}
          <div style={{ color: C.ash }} className="fs-10 mt-2">¿Tienes material propio (fotos de proyectos, renders)? Súbelo con el botón <b style={{ color: C.mist }}>"Subir"</b> del publicador.</div>
        </div>
              ...
              <div key={x.id} style={{ background: C.panel, border: `1px solid ${C.line}` }} className="rounded-xl overflow-hidden">
                <div style={{ background: "#000", aspectRatio: "9/16", maxHeight: 300 }} className="flex items-center justify-center relative">
                  {x.estado === "listo" && x.tipo === "video" && x.video ? <video src={x.video} controls style={{ width: "100%", height: "100%", objectFit: "contain" }} />
                    ...
                    : x.tipo === "carrusel" && (x.slides || []).length ? <><div style={{ width: "100%", height: "100%" }} dangerouslySetInnerHTML={{ __html: x.slides[0].svg.replace("<svg", '<svg style="width:100%;height:100%;object-fit:contain"') }} /><span style={{ position: "absolute", top: 6, right: 6, background: "rgba(11,10,8,0.8)", border: `1px solid ${C.aetherLine}`, color: C.aether2, fontFamily: MONO }} className="fs-9 px-1.5 py-0.5 rounded">⧉ {x.slides.length} láminas</span></>
