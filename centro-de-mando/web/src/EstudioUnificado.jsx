...
import { Flame, SquarePlay as Youtube, Image as ImageIcon, Scissors, Compass, Wand2, Share2 } from "lucide-react";
...
// Módulo unificado en 3 categorías: Identificar (tendencia) → Crear contenido
// (pieza) → Publicar (redes/historial). Espejo del sistema original,
// parametrizado por workspace (data[ws]).
export default function EstudioUnificado({ data, commit, ws, recargar }) {
  ...
  const [subIdent, setSubIdent] = useState("viral");
  ...
  const nPub = (data[ws]?.publicaciones || []).length;
  const nGuard = (data[ws]?.contenidos || []).length;
  ...
  const props = { data, commit, ws, recargar, flash };
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      ...
      {aviso && (
        <div style={{ background: C.aetherSoft, border: `1px solid ${C.aetherLine}`, color: C.aether2 }} className="rounded-lg px-3 py-2 mb-4 fs-12">{aviso}</div>
      )}
      ...
      <div style={{ display: top === "identificar" && subIdent === "viral" ? "block" : "none" }}>
        <ViralView {...props} irAEstudio={irAEstudio} />
      </div>

      {/* ContenidoView (única instancia): YouTube dentro de Identificar + todo Publicar */}
      <div style={{ display: contenidoVisible ? "block" : "none" }}>
        <ContenidoView {...props} pubDraft={pubDraft} clearPubDraft={() => setPubDraft(null)} irAEstudio={irAEstudio} subtab={contenidoSubtab} setSubtab={setSubPublicar} ocultarBarra />
      </div>

      {/* Crear contenido · Imagen/Video/Diseño */}
      <div style={{ display: top === "crear" && subCrear === "estudio" ? "block" : "none" }}>
        <EstudioContenidoView {...props} onPublicar={irAPublicar} estudioDraft={estudioDraft} clearEstudioDraft={() => setEstudioDraft(null)} irAEditor={irAEditor} />
      </div>
      {/* Crear contenido · Editor de video */}
      <div style={{ display: top === "crear" && subCrear === "editor" ? "block" : "none" }}>
        <VideoEditor {...props} editorSrc={editorSrc} onPublicar={irAPublicar} />
      </div>
    </div>
  );
}
