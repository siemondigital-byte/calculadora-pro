...
const C = {
  ...
  aether: "#E6C788", aether2: "#EFD9A7", aether500: "#B39355", aetherSoft: "rgba(230,199,136,0.14)",
...
const inS = { background: "#0D1420", border: `1px solid ${C.line}`, color: C.cream, borderRadius: 10, padding: "9px 12px", width: "100%", fontFamily: SANS, fontSize: 13, outline: "none" };
const H = () => ({ "content-type": "application/json", Authorization: "Bearer " + getToken() });
const uid = () => (crypto?.randomUUID ? crypto.randomUUID() : Math.random().toString(36).slice(2, 9));
const hoy = () => new Date().toISOString().slice(0, 10);

const URLS_AUDITAR = ["https://atlantisglobalrealty.com/", "https://cicloderiqueza.atlantisglobalrealty.com/", "https://wealthcycle.atlantisglobalrealty.com/"];
...
export default function BlogSeoView({ data, commit, ws }) {
  const [avisoLocal, setAvisoLocal] = useState("");
  const flash = (m) => { setAvisoLocal(m); window.clearTimeout(flash._t); flash._t = window.setTimeout(() => setAvisoLocal(""), 6000); };
  const [tab, setTab] = useState("blog");
  const articulos = data[ws].blogArticulos || [];

  // ---- SEO ----
  const [url, setUrl] = useState("https://atlantisglobalrealty.com/");
  const [comp, setComp] = useState("");
  const [kw, setKw] = useState("");
  const [busySeo, setBusySeo] = useState(false);
  const [catAbierta, setCatAbierta] = useState("");
  const audit = data[ws].seoAudit || null;
  ...
  async function auditar() {
    ...
    try {
      ...
      commit({ ...data, [ws]: { ...data[ws], seoAudit: { ...d, fecha: hoy(), keyword: kw } } });
  ...
  const kwData = data[ws].blogKeywords || null;         // {seed, pais, keywords, fuente, nota, fecha}
  const gscData = data[ws].gscConsultas || null;        // {consultas, fecha}
  ...
  async function investigarKeywords(seedOverride) {
    ...
    try {
      ...
      commit({ ...data, [ws]: { ...data[ws], blogKeywords: { ...d, fecha: hoy() } } });
  ...
  const curadas = data[ws].blogKeywordsCuradas || [];
  ...
  function setCuradas(nuevas) { commit({ ...data, [ws]: { ...data[ws], blogKeywordsCuradas: nuevas } }); }
  ...
  async function sugerirKeywords() {
    ...
    try {
      const r = await fetch(MOTOR + "/blog/sugerir_keywords", { method: "POST", headers: H(), body: JSON.stringify({ curadas: (data[ws].blogKeywordsCuradas || []), contexto_mercado: insightsMercado(data), evitar: sugeridasRef.current }) });
      const d = await r.json();
      ...
      if (d.ok && d.conectado) {
        ...
        fetch(MOTOR + "/blog/gsc_sitios", { headers: H() }).then((r) => r.json()).then((s) => {
          if (s.ok) {
            ...
            const pref = (s.usables || []).find((x) => /atlantisglobalrealty\.com/.test(x.sitio)) || (s.usables || [])[0] || (s.sitios || [])[0];
  ...
  async function actualizarGsc() {
    ...
    try {
      const r = await fetch(MOTOR + "/blog/gsc_actualizar", { method: "POST", headers: H(), body: JSON.stringify({ dominio: "atlantisglobalrealty.com", sitio: gscSitio }) });
      const d = await r.json();
      if (!d.ok) {
      ...
      commit({ ...data, [ws]: { ...data[ws], gscConsultas: { consultas: d.consultas, fecha: d.fecha } } });
  ...
  function patchArts(nuevos) { commit({ ...data, [ws]: { ...data[ws], blogArticulos: nuevos } }); }
  async function generarIdeas(seedOverride) {
    ...
    try {
      const r = await fetch(MOTOR + "/blog/ideas", { method: "POST", headers: H(), body: JSON.stringify({
        ...
        curadas: (data[ws].blogKeywordsCuradas || []) }) });
      const d = await r.json();
      ...
      if ((d.keywords || []).length) commit({ ...data, [ws]: { ...data[ws], blogArticulos: [...nuevas, ...articulos], blogKeywords: { seed: (seedKw || temaBlog), pais: paisKw, keywords: d.keywords, fuente: d.fuente_keywords, nota: d.nota_keywords, fecha: hoy() } } });
  ...
  const blogCfg = data[ws].blogConfig || {};
  function setBlogCfg(patch) { commit({ ...data, [ws]: { ...data[ws], blogConfig: { ...blogCfg, ...patch } } }); }
  ...
  function abrirArticulo(a) {
    ...
    window.open("https://atlantisglobalrealty.com/blog/?p=" + slug, "_blank");
  }
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      {avisoLocal && (
        <div style={{ background: C.aetherSoft, border: `1px solid ${C.aetherLine}`, color: C.aether2, position: "sticky", top: 8, zIndex: 30 }} className="rounded-xl p-3 mb-4 fs-12">{avisoLocal}</div>
      )}
      <div className="mb-4">
        ...
        <div style={{ color: C.ash }} className="fs-11 mt-1">Audita el SEO de tu web (y de la competencia) con verificaciones reales, y produce artículos de blog por keyword. Los artículos con fecha aparecen en el Calendario, y los "publicado" salen en atlantisglobalrealty.com/blog. Cada lunes el sistema propone el artículo "listo" con mejor SEO y te llega un correo para aprobarlo: al dar OK se publica, sale la newsletter y se comenta en tus redes.</div>
      </div>
      ...
      {tab === "blog" && (
        <div>
          {(() => { const objs = [...(data[ws].blogKeywordsCuradas || [])].filter((c) => c.objetivo).sort((a, b) => (b.volumen || 0) - (a.volumen || 0)); return (
                  ...
                  <div key={a.id} style={{ background: C.panel, border: `1px solid ${C.line}` }} className="rounded-xl p-3.5">
                    <div className="flex items-start gap-3 flex-wrap">
                      ...
                      <div className="flex items-center gap-1.5 shrink-0 flex-wrap justify-end">
                        ...
                        {a.cuerpo_md && a.estado !== "publicado" && <button onClick={() => setArt(a.id, { estado: "publicado", fechaPublicacion: a.fechaPublicacion || hoy() })} title="Publica el artículo en atlantisglobalrealty.com/blog ahora mismo" style={{ background: C.ok, color: C.obsidian, fontWeight: 600 }} className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg fs-11">✓ Publicar</button>}
