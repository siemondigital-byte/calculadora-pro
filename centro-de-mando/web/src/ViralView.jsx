...
const NIVEL_COLOR = { "0-1": "var(--marca-ok)", "2-3": "var(--marca-acento-claro)", "4": "var(--marca-aviso)" };
...
export default function ViralView({ data, commit, ws, flash, irAEstudio }) {
  const ideas = data[ws].viralIdeas || [];
  ...
  function patch(nuevas) { commit({ ...data, [ws]: { ...data[ws], viralIdeas: nuevas } }); }
  ...
  async function generarLote() {
    ...
    try {
      for (let tanda = 1; tanda <= 2; tanda++) {
        const r = await fetch(MOTOR + "/viral/ideas", { method: "POST", headers: H(), body: JSON.stringify({ n: 10, tema, contexto_mercado: insightsMercado(data), curadas: (data[ws].blogKeywordsCuradas || []) }) });
  ...
  return (
    <div style={{ maxWidth: 1100 }}>
      <div className="mb-4">
        ...
        <div style={{ color: C.ash }} className="fs-11 mt-1 leading-snug">Las redes son para entretener: engancha primero (amplio, nivel 0-1), enseña en el medio, y conecta al final con el método. Genera ideas, filtra las mejores, saca su guion de 5 partes, y aplica el 80/20: mata lo que no funciona y dobla la apuesta en lo que sí.</div>
      </div>

      {/* generador */}
      <div style={{ background: C.panel, border: `1px solid ${C.line}` }} className="rounded-xl p-4 mb-4">
        <div className="flex items-center gap-2 flex-wrap">
          <input style={{ ...inS, maxWidth: 420 }} value={tema} onChange={(e) => setTema(e.target.value)} placeholder="Enfoque del lote (opcional, ej. profesionales que ahorran sin plan)" />
