...
const URLS_VIVO = {
  "index.html": "https://atlantisglobalrealty.com/",
  "styles.css": "https://atlantisglobalrealty.com/",
};

export default function MaquetadorView({ data, commit, ws }) {
  ...
  useEffect(() => {
    if (chkSynced) return;
    const s = (data[ws]?.saludWeb && data[ws]?.saludWeb.soluciones) || {};
    if (!s.title_propuesto && !s.description_propuesta && !(s.alts || []).length) { setChkSynced(true); return; }
    (async () => {
      try {
        const r = await fetch(MOTOR + "/web/aplicar_soluciones", { method: "POST", headers: H(), body: JSON.stringify({ dry_run: true, soluciones: (data[ws]?.saludWeb || {}).soluciones }) });
        const d = await r.json();
        if (d && d.ok) {
          const pend = d.detalle || [];
          const hoy = new Date().toISOString().slice(0, 10);
          const ck = { ...(data[ws]?.checklistWeb || {}) };
          let changed = false;
          ...
          if (changed) commit({ ...data, [ws]: { ...data[ws], checklistWeb: ck } });
        }
      } catch {}
  ...
  async function aplicarSoluciones() {
    setBusy("sol");
    try {
      const r = await fetch(MOTOR + "/web/aplicar_soluciones", { method: "POST", headers: H(), body: JSON.stringify({ soluciones: (data[ws]?.saludWeb || {}).soluciones }) });
      const d = await r.json();
      ...
      commit({ ...data, [ws]: { ...data[ws], checklistWeb: ck } });
  ...
  return (
    <div style={{ maxWidth: 1000 }}>
      {aviso && (
        <div style={{ background: C.aetherSoft, border: `1px solid ${C.aetherLine}`, color: C.aether2, position: "sticky", top: 8, zIndex: 30 }} className="rounded-xl p-3 mb-4 fs-12">{aviso}</div>
      )}
      <div className="mb-5">
      ...
      {(() => {
        const sols2 = (data[ws]?.saludWeb && data[ws]?.saludWeb.soluciones) || {};
        const audit = data[ws]?.auditoriaNegocio || {};
        ...
        const ck = data[ws]?.checklistWeb || {};
        const marcar = (k) => commit({ ...data, [ws]: { ...data[ws], checklistWeb: { ...ck, [k]: ck[k] ? undefined : new Date().toISOString().slice(0, 10) } } });
