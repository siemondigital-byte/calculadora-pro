...
import { Youtube, Sparkles, Copy, Check, FileText, Type, Image as ImageIcon, Repeat, CalendarDays, TrendingUp, RefreshCw, ArrowRight, AtSign } from "lucide-react";
...
const PILARES = [
  "IA y automatización aplicada a negocios",
  "Emprender y escalar servicios sin quemarte",
  "Desde el ser: mentalidad, método y propósito",
];
...
export default function EstudioYtView({ data, commit, flash }) {
  ...
  const ideas = data.siemon.ideasTendencia || [];   // persistido: no se pierde al cambiar de pestaña
  ...
  const pilaresTend = (data.siemon.pilaresTendencia && data.siemon.pilaresTendencia.length) ? data.siemon.pilaresTendencia : PILARES_TEND_DEFAULT;
  ...
  // analítica del canal
  const [canalHandle, setCanalHandle] = useState("@andreasiemon-l2u");
  const [analitica, setAnalitica] = useState(null);
  const [anBusy, setAnBusy] = useState(false);
  // OAuth analítica privada
  const [sec, setSec] = useState({ YT_OAUTH_CLIENT: false, YT_ANALYTICS: false });
  const [cid, setCid] = useState("");
  const [cs, setCs] = useState("");
  const [showCred, setShowCred] = useState(false);
  const [priv, setPriv] = useState(null);
  const [privBusy, setPrivBusy] = useState(false);
  const guardados = (data.siemon.contenidos || []).filter((x) => x.red === "youtube");

  async function cargarSec() {
    try { const r = await fetch(MOTOR + "/config/secretos", { headers: { Authorization: "Bearer " + getToken() } }); setSec(await r.json()); } catch {}
  }
  useEffect(() => { cargarSec(); }, []);
  async function guardarCliente() {
    ...
    try {
      await fetch(MOTOR + "/config/secreto", { method: "POST", headers: { "content-type": "application/json", Authorization: "Bearer " + getToken() }, body: JSON.stringify({ clave: "YT_OAUTH_CLIENT_ID", valor: cid.trim() }) });
      await fetch(MOTOR + "/config/secreto", { method: "POST", headers: { "content-type": "application/json", Authorization: "Bearer " + getToken() }, body: JSON.stringify({ clave: "YT_OAUTH_CLIENT_SECRET", valor: cs.trim() }) });
      setCid(""); setCs(""); flash("Credenciales guardadas. Ahora conecta tu YouTube."); cargarSec();
    } catch { flash("No pude guardar."); }
  }
  function conectarYt() { window.open(MOTOR + "/oauth/youtube/start?k=" + encodeURIComponent(getToken()), "_blank"); flash("Autoriza en la ventana de Google, luego vuelve y dale 'Ver métricas'."); }
  async function verPrivada() {
    setPrivBusy(true);
    try {
      const r = await fetch(MOTOR + "/canal_analitica_privada", { method: "POST", headers: { "content-type": "application/json", Authorization: "Bearer " + getToken() }, body: JSON.stringify({}) });
  ...
  async function buscarIdeas() {
    ...
    try {
      ...
      const r = await fetch(MOTOR + "/ideas", { method: "POST", headers: { "content-type": "application/json", Authorization: "Bearer " + getToken() },
  ...
  async function analizarCanal() {
    setAnBusy(true);
    try {
      const r = await fetch(MOTOR + "/canal_analitica", { method: "POST", headers: { "content-type": "application/json", Authorization: "Bearer " + getToken() },
        body: JSON.stringify({ handle: canalHandle }) });
      const d = await r.json();
      if (d.ok) setAnalitica(d);
      else flash("Analítica: " + (d.error || "no pude"));
    } catch { flash("No pude conectar con el motor."); }
  ...
  function guardar() {
    ...
    commit({ ...data, siemon: { ...data.siemon, contenidos: [item, ...(data.siemon.contenidos || [])] } });
    flash("Guardado en Contenido.");
  }
  ...
  return (
    <div className="p-5 md:p-8" style={{ maxWidth: 1100 }}>
      <div className="mb-5">
        ...
        <h1 style={{ fontWeight: 600, color: C.cream }} className="fs-24 flex items-center gap-2"><Youtube size={20} color={C.aether} /> Estudio de tu canal</h1>
        <div style={{ color: C.ash }} className="fs-11 mt-1">Guiones con retención, títulos, briefs de miniatura, repurpose multicanal y calendario, en tu voz de marca y tus pilares.</div>
      </div>
      ...
      <div style={{ background: C.panel, border: `1px solid ${C.line}` }} className="rounded-xl p-4 mb-4">
        ...
        {showPil && (
          <div style={{ background: C.carbon, border: `1px solid ${C.line}` }} className="rounded-lg p-3 mb-2">
            <div style={{ color: C.ash }} className="fs-10 mb-2">Edita los términos de búsqueda (español e inglés). Uno por pilar.</div>
            {pilEdit.map((p, i) => (
      ...
      <div style={{ background: C.panel, border: `1px solid ${C.line}` }} className="rounded-xl p-4 mb-4">
        ...
        {!analitica && <div style={{ color: C.ash }} className="fs-11">Mira qué videos rinden mejor en tu canal (datos públicos: vistas, likes). Retención, CTR y fuentes de tráfico necesitan conectar tu canal por OAuth.</div>}
        {analitica && (
          <div>
            ...
            {analitica.videos.length === 0 ? (
              <div style={{ color: C.ash }} className="fs-11">Tu canal aún no tiene videos públicos. Cuando publiques, aquí verás cuáles funcionan mejor.</div>
            ) : (
        ...
        <div style={{ borderTop: `1px solid ${C.line}` }} className="mt-3 pt-3">
          ...
          {/* credenciales: SIEMPRE editables (para corregir si se pegó mal) */}
          <div className="mb-2">
            <div className="flex items-center justify-between">
              <span style={{ color: C.ash }} className="fs-11">{sec.YT_OAUTH_CLIENT ? "Credenciales de Google guardadas" : "Pega tus credenciales de Google Cloud (OAuth Client tipo Web)"}</span>
              <button onClick={() => setShowCred((v) => !v)} style={{ color: C.aether2 }} className="fs-10 underline">{showCred ? "cerrar" : (sec.YT_OAUTH_CLIENT ? "cambiar" : "configurar")}</button>
            </div>
            {sec.YT_OAUTH_CLIENT && (sec.yt_client_id_valido === false || sec.yt_secret_valido === false) && (
              <div style={{ background: "rgba(224,122,95,0.12)", border: "1px solid rgba(224,122,95,0.4)", color: "#E8B4A0" }} className="rounded-lg p-2.5 mt-2 fs-10 leading-relaxed">
                <b>Lo guardado no sirve, por eso falla la conexión:</b><br />
                Client ID: <span style={{ fontFamily: MONO }}>{sec.yt_client_id_preview}</span> {sec.yt_client_id_valido ? "✓" : "✗ debe terminar en .apps.googleusercontent.com"}<br />
                Secret: <span style={{ fontFamily: MONO }}>{sec.yt_secret_preview}</span> {sec.yt_secret_valido ? "✓" : "✗ debe empezar por GOCSPX-"}<br />
                Pulsa "cambiar" y pega los valores reales de Google Cloud Console.
              </div>
            )}
            {(showCred || !sec.YT_OAUTH_CLIENT) && (
              <div className="mt-2">
                <div className="grid sm:grid-cols-2 gap-2">
                ...
                <div style={{ color: C.warn }} className="fs-9 mt-1">Importante: el Client ID NO es tu correo. Es un texto largo que termina en <b>.apps.googleusercontent.com</b> y el Secret empieza con <b>GOCSPX-</b>.</div>
              </div>
            )}
          </div>
          {sec.YT_OAUTH_CLIENT && !sec.YT_ANALYTICS && (
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span style={{ color: C.ash }} className="fs-10">Autoriza con tu cuenta; se abre Google en otra ventana.</span>
      ...
      <div style={{ background: C.panel, border: `1px solid ${C.line}` }} className="rounded-xl p-4 md:p-5 mb-5">
        <div className="grid md:grid-cols-2 gap-3">
          <div className="md:col-span-2"><label style={{ color: C.ash, fontFamily: MONO }} className="fs-10 uppercase">Tema del video {accion === "calendario" ? "(opcional)" : "(elige uno o escribe el tuyo)"}</label>
            <Combo listaId="dl-tema-yt" style={inS} value={tema} onChange={setTema} opciones={opcionesDe(data, "temaYt", [...PILARES, ...pilaresTend.map((p) => p.es)])} placeholder="ej. cómo automatizar la atención al cliente con IA" /></div>
