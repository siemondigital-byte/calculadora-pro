...
import { ChevronLeft, ChevronRight, Trash2, RefreshCw, CalendarDays } from "lucide-react";

const C = {
  panel: "#16171C", carbon: "#131418", line: "rgba(255,255,255,0.08)",
...
// Calendario de publicación: muestra publicaciones (programadas y publicadas) por día.
export default function CalendarioView({ data, commit, flash, irACrear }) {
  const hoy = new Date();
  const [ym, setYm] = useState({ y: hoy.getFullYear(), m: hoy.getMonth() });
  const [sel, setSel] = useState("");
  const pubs = data.siemon.publicaciones || [];
  // artículos de blog con fecha: también viven en el calendario (📝)
  const blogs = (data.siemon.blogArticulos || []).filter((a) => a.fechaPublicacion);
  ...
  const borrar = (id) => { commit({ ...data, siemon: { ...data.siemon, publicaciones: pubs.filter((x) => x.id !== id) } }); flash("Eliminada."); };
  const cambiarFecha = (id, fecha) => { commit({ ...data, siemon: { ...data.siemon, publicaciones: pubs.map((x) => x.id === id ? { ...x, fecha, estado: x.estado === "Publicada" ? x.estado : "Programada" } : x) } }); flash("Fecha actualizada."); };
  ...
  return (
    <div className="p-5 md:p-8" style={{ maxWidth: 1100 }}>
      ...
      {sel && (() => { const p = pubs.find((x) => x.id === sel); if (!p) return null; return (
        <div style={{ background: C.panel, border: `1px solid ${C.aetherLine}` }} className="rounded-xl p-4 mb-4">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {irACrear && <button onClick={() => irACrear(p)} style={{ color: C.aether2 }} className="fs-11 inline-flex items-center gap-1"><RefreshCw size={12} /> Editar / republicar</button>}
      ...
      <div>
        ...
        {programadas.length === 0 ? <div style={{ color: C.ash }} className="fs-11">Nada programado aún. En "Crear y publicar" genera un post y usa "Programar".</div> : (
